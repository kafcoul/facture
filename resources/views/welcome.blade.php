            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8">
                <div class="reveal text-center" style="transition-delay:0ms">
                    ...
                    <div class="text-3xl font-extrabold text-deep-900 counter"
                        data-target="{{ $stats['users'] ?? 500 }}">0</div>
                ...
                <div class="reveal text-center" style="transition-delay:80ms">
                    ...
                    <div class="text-3xl font-extrabold text-deep-900 counter"
                        data-target="{{ $stats['tenants'] ?? 120 }}">0</div>
                ...
                <div class="reveal text-center" style="transition-delay:160ms">
                    ...
                    <div class="text-3xl font-extrabold text-deep-900 counter"
                        data-target="{{ $stats['invoices'] ?? 12500 }}">0</div>
                ...
                <div class="reveal text-center" style="transition-delay:240ms">
                    ...
                    <div class="text-3xl font-extrabold text-deep-900"><span class="counter"
                            data-target="{{ intval(($stats['total_invoiced'] ?? 850000000) / 1000000) }}">0</span>M
                    </div>
    ...
    <section class="py-12 bg-surface">
        <div class="max-w-6xl mx-auto px-5 sm:px-6">
            ...
            <div class="reveal flex flex-wrap justify-center gap-3" style="transition-delay:80ms">
                @foreach (['Orange Money', 'Wave', 'MTN MoMo', 'Moov Money', 'Free Money', 'M-Pesa', 'Visa', 'Mastercard', 'PayPal', 'Stripe', 'CinetPay', 'PayDunya'] as $provider)
                    <span
                        class="payment-pill inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-white border border-slate-100 text-xs font-medium text-slate-500 shadow-sm hover:border-brand-200 hover:text-brand-600 transition-all">{{ $provider }}</span>
                @endforeach
            </div>
        </div>
    </section>

    {{-- ====== POURQUOI INVOICESAAS (Problème → Solution) ====== --}}
                ...
                @foreach ($faqs as $i => $faq)
                    <div class="reveal faq-item bg-white rounded-xl border border-slate-100 overflow-hidden transition-all"
                        ...
                        <button onclick="toggleFaq(this)"
                            ...
                            <span
                                class="flex-shrink-0 w-8 h-8 rounded-lg bg-brand-50 flex items-center justify-center text-sm font-bold text-brand-600 group-hover:bg-brand-100 transition-colors">{{ $i + 1 }}</span>
                            <span class="flex-1 text-sm font-semibold text-slate-700">{{ $faq['q'] }}</span>
                        ...
                        <div class="faq-answer px-6 pb-0 max-h-0 overflow-hidden transition-all duration-300">
                            <p class="text-sm text-slate-400 leading-relaxed pb-4 pl-12">{{ $faq['a'] }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="modal-content relative w-full max-w-lg mx-4 max-h-[90vh]">
            <div class="bg-white rounded-3xl shadow-elevated overflow-hidden overflow-y-auto max-h-[90vh]">
                ...
                <div class="px-8 py-6">
                    ...
                    {{-- Form --}}
                    <form id="registerForm" action="{{ route('register.with-plan') }}" method="POST">
                        @csrf
