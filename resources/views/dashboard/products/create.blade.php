...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <!-- Form -->
    <form action="{{ route('client.products.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 divide-y divide-gray-200">
        @csrf
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <label class="relative flex cursor-pointer rounded-lg border bg-white p-4 shadow-sm focus:outline-none" 
                       ...
                       x-data="{ type: '{{ old('type', 'product') }}' }">
                    <input type="radio" name="type" value="product" class="sr-only" x-model="type" {{ old('type', 'product') === 'product' ? 'checked' : '' }}>
                ...
                <label class="relative flex cursor-pointer rounded-lg border bg-white p-4 shadow-sm focus:outline-none"
                       x-data="{ type: '{{ old('type', 'product') }}' }">
                    <input type="radio" name="type" value="service" class="sr-only" x-model="type" {{ old('type') === 'service' ? 'checked' : '' }}>
                    <span class="flex flex-1">
                        <span class="flex flex-col">
                            <span class="flex items-center space-x-2">
