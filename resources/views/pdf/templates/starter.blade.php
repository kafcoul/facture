...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .info-block h3 { font-size: 11px; color: #6b7280; text-transform: uppercase; margin-bottom: 6px; font-weight: 600; }
        .info-block p { margin: 2px 0; font-size: 11px; }
        
        .items-table { width: 100%; border-collapse: collapse; margin-bottom: 25px; }
        .items-table thead { background-color: #f9fafb; }
        .items-table th { padding: 10px 8px; text-align: left; font-weight: 600; font-size: 10px; text-transform: uppercase; color: #6b7280; border-bottom: 1px solid #e5e7eb; }
        .items-table td { padding: 10px 8px; border-bottom: 1px solid #f3f4f6; font-size: 11px; }
        ...
        .totals-row .label { display: table-cell; text-align: right; padding-right: 15px; font-size: 11px; }
        .totals-row .value { display: table-cell; text-align: right; width: 100px; font-size: 11px; }
        .totals-row.total { font-size: 14px; font-weight: 700; color: #1f2937; padding-top: 8px; border-top: 2px solid #e5e7eb; margin-top: 8px; }
        ...
        .notes { margin-top: 20px; padding: 12px; background-color: #f9fafb; border-left: 3px solid #6b7280; font-size: 10px; }
        .notes h4 { font-size: 10px; margin-bottom: 5px; color: #6b7280; font-weight: 600; }
        
        .footer { margin-top: 40px; padding-top: 15px; border-top: 1px solid #e5e7eb; text-align: center; font-size: 9px; color: #9ca3af; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="info-section">
            <div class="info-column">
                <div class="info-block">
                    <h3>De</h3>
                    <p><strong>{{ $invoice->user->company_name ?? $invoice->user->name ?? config('app.name') }}</strong></p>
            ...
            <div class="info-column">
                ...
                <div class="info-block">
                    <table style="width: 100%;">
                        <tr><td style="padding: 2px 0; font-size: 10px;"><strong>Date:</strong></td><td style="text-align: right; font-size: 10px;">{{ $invoice->issue_date->format('d/m/Y') }}</td></tr>
                        <tr><td style="padding: 2px 0; font-size: 10px;"><strong>Échéance:</strong></td><td style="text-align: right; font-size: 10px;">{{ $invoice->due_date->format('d/m/Y') }}</td></tr>
                        <tr><td style="padding: 2px 0; font-size: 10px;"><strong>Statut:</strong></td><td style="text-align: right;"><span class="status-badge status-{{ $invoice->status }}">{{ ucfirst($invoice->status) }}</span></td></tr>
                    </table>
                </div>
            </div>
        </div>
        ...
        @if($invoice->notes)<div class="notes"><h4>Notes</h4><p>{{ $invoice->notes }}</p></div>@endif
